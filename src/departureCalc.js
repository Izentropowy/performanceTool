...
export function calcTorAndTod(
  ...
  let pressReducedLowTemp = [
    tempReduced[0][pressFloor / 1000],
    tempReduced[0][pressCeil / 1000],
  ];
  // tables only with required pressures for higher temp
  let pressReducedHighTemp = [
    tempReduced[1][pressFloor / 1000],
    tempReduced[1][pressCeil / 1000],
  ];
  // calc TORs and TODs for respective temperatures
  let lowerTor = interpolate(
    pressFloor,
    pressCeil,
    pressReducedLowTemp[0][2],
    pressReducedLowTemp[1][2],
    pressAlt
  );
  let higherTor = interpolate(
    pressFloor,
    pressCeil,
    pressReducedHighTemp[0][2],
    pressReducedHighTemp[1][2],
    pressAlt
  );
  let lowerTod = interpolate(
    pressFloor,
    pressCeil,
    pressReducedLowTemp[0][3],
    pressReducedLowTemp[1][3],
    pressAlt
  );
  let higherTod = interpolate(
    pressFloor,
    pressCeil,
    pressReducedHighTemp[0][3],
    pressReducedHighTemp[1][3],
    pressAlt
  );
  // calc TOR
...
export function calcRoc(qnh, elevation, cruise, temperature) {
  ...
  let pressAltCruise =
    parseFloat(pressAltTakeoff) + parseFloat(cruise) - elevation;
  ...
  // tables only with required pressures for lower temp
  let pressReducedLowTemp = [
    tempReduced[0][pressFloor / 2000],
    tempReduced[0][pressCeil / 2000],
  ];
  // tables only with required pressures for higher temp
  let pressReducedHighTemp = [
    tempReduced[1][pressFloor / 2000],
    tempReduced[1][pressCeil / 2000],
  ];
  // calc ROCs for respective temperatures
  let lowerRoc = interpolate(
    pressFloor,
    pressCeil,
    pressReducedLowTemp[0][2],
    pressReducedLowTemp[1][2],
